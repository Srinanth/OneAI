...
export class GrokAdapter implements AIModelAdapter {
  public id: string;
  public maxTokens = 131000;
  ...
  public async sendMessage(messages: Message[], currentArtifact: Artifact, apiKey: string): Promise<AIResponse> {
    if (!apiKey) throw new Error("xAI (Grok) API Key is missing.");
    ...
    try {
      ...
      console.error(`Grok Adapter (${this.id}) Error:`, error);
      throw new Error(error?.message || "xAI API Error");
    }
  }
}